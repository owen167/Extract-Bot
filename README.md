...
The primary detector is [`ogkalu/comic-text-and-bubble-detector`](https://huggingface.co/ogkalu/comic-text-and-bubble-detector). Its small `detector-v4-s_int8.onnx` checkpoint is loaded with ONNX Runtime and recognizes `bubble`, `text_bubble`, and `text_free`. Long webtoon pages are processed as overlapping vertical tiles instead of being squeezed into one 640px image. Text inside a bubble maps to `SPEECH`; text outside a bubble maps to `SIDE_TEXT`; a detected bubble without a matching text box is treated as ordinary speech so text is not lost. Unknown or ambiguous bubble shapes intentionally fall back to ordinary speech.
